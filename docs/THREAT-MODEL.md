...
| Hostile process under the selected Unix account | User-owned installer staging, installed runtime, and instance state are mutable by that identity; installer byte/hash rechecks are defense in depth, not an atomic native-inode launch guarantee on both Linux and macOS | Treat as account/host compromise. Stop the service, restore a known-good runtime and state, rotate secrets and tokens as applicable, and do not rely on the one-line installer to recover trust while the account remains compromised. |
...
the private key, requires an exact match, and recomputes the exact OpenSSH
